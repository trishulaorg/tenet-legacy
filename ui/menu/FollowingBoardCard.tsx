...
const FollowingBoardCard: FC<FollowingBoardCardProps> = ({ boards }) => {
  ...
  return (
    <div className="rounded overflow-hidden shadow-lg my-2 py-2 px-2 bg-contentbg">
      <table>
        <tbody>
          <tr className="font-bold text-xl text-fontcl-heavy mb-2">Following Boards:</tr>
          {boards.map((board) => (
            <tr className="text-fontcl-med text-base" key={`following_${board.board.id}`}>
